...
weightslist = sorted( glob.glob( filepath+'bottom-band-weight-Ef-*.txt') )

# calculate weight/projection on 0th order Block
weight = np.zeros( (nEf, nr*nm) )
for i, weightsfilename in enumerate(weightslist):
...
for i in range(nEf):
  ...
  for j in range(nE):
    gauswE[j, i] = norm*np.sum(weight[i,:]*np.exp( -(E[j] - energy[:,i])**2 / sig2))
