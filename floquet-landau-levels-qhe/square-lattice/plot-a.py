...
gE = np.zeros((nphi-1,nE))
for i in range(nphi):
  for j in range(nE-1):
    #idx = np.where(np.logical_and(energy[:,i]>E[j],energy[:,i]<E[j+1]))[0]
    idx = np.where(np.logical_and(eng_k[:,i+1]>E[j],eng_k[:,i+1]<E[j+1]))[0]
...
#  plt.imshow(gE.transpose(), origin='lower', extent=Extent, cmap='viridis', aspect=3/5)
plt.imshow(gE.transpose(), cmap='viridis',interpolation='spline16', origin='lower', extent=Extent, aspect=0.5)
