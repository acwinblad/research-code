...
groundStates = np.zeros(np.size(eta))
for k in xrange(np.size(eta)):
  ...
  for i in xrange(nr):
    for j in xrange(i+1):
      ...
      ftr[latticeCtr] = (1./(nr*a*np.sqrt(3)/2.))*siteCoord[latticeCtr,1]*eta[k]*mu
      latticeCtr += 1
  ...
  groundStates[k] = np.min(np.abs(energy))

np.savetxt('../../data/bdg-spectral-flow-mu-2.txt', np.column_stack([eta,groundStates]), fmt='%1.8f')
