...
n = nr*(nr+1)/2
# slope of delta 
ddel = (delta-deltamin)/(nr*a*np.sqrt(3)/2)
...
for i in xrange(nr):
  for j in xrange(i+1):
    ...
    ftr[latticeCtr] = ddel*i+deltamin
    latticeCtr += 1
...
for i in xrange(n):
  for j in xrange(n-i):
    ...
    if d < 1e-5:
      ...
      # since we start with the bottom at y=0 and move up to the vertex at x=0
      # we want to have a factor that is decreasing as y increases
      bdg[i+j,i] = -thop
      bdg[i,i+j] = -thop
      bdg[i+j+n,i+n] = thop
      bdg[i+n,i+j+n] = thop
      bdg[i+j,i+n] = ftr[i]*np.exp(1j*phaseAngle)
      bdg[i,i+j+n] = -ftr[i]*np.exp(1j*phaseAngle)
...
np.savetxt('../../data/bdg-energy-linear-inc-delta.txt', energy, fmt='%1.8f')
np.savetxt('../../data/bdg-states-linear-inc-delta.txt', states, fmt='%1.32f')
np.savetxt('../../data/bdg-coord-linear-inc-delta.txt', siteCoord, fmt='%1.32f')
