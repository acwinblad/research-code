...
def plot_individual_comparison(_idx, _triang, _energy, _states, _n):
  ...
  for i in _idx:
    ...
    filename = '../../data/fig-wavefunction-energy-linear-inc-mu-%02d.pdf' % (_n-1-i)
...
def plot_subgroup(_idx, _triang, _energy, _states, _n, _name):
  ...
  filename = '../../data/fig-wavefunction-energy-linear-inc-mu-%s.pdf' % _name
  plt.savefig(filename)
  plt.close()

# create triangular mesh
x,y = np.loadtxt('../../data/bdg-coord-linear-inc-mu.txt', unpack=True)
n = np.size(x)
triang = mtri.Triangulation(x,y)
# load in eigen- energy and states
energy = np.loadtxt('../../data/bdg-energy-linear-inc-mu.txt')
states = np.loadtxt('../../data/bdg-states-linear-inc-mu.txt')
...
if plot_edge_states==True:
  #energyBounds = np.loadtxt('../../data/energy-band-gap-bounds.txt')
  energyBounds = np.array([-6.,6.])
