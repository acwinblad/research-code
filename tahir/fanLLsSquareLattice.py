...
def hjjn(_n, _i, _j, _phi0, _kj, _ja):
  cs = np.cos(_kj)
  if _i == _j:
    return -(sp.jv(_n,_phi0*cs)*np.exp(1.0j*_ja)+sp.jv(_n,-_phi0*cs)*np.exp(-1.0j*_ja)) * np.exp(1.0j*_n*np.pi/2)
  elif _i+1 == _j:
  ...
  for n in range(Nm):
    for i in range(Ns):
      for j in range(Ns):
        #kj = (j%Ns)*ka
        kj = (j-rc)*ka
        Hn[n,i,j] = hjjn(-n, i, j, phi0[k], kj, ja)
#        if k==0:
#          print(Hn[n,i,j])
