...
def hjjn(_n, _i, _j, _phi0, _kj, _kya):
  cs = np.cos(_kj)
  if _i == _j:
    return -(sp.jv(_n,_phi0*cs)*np.exp(1.0j*_kya)+sp.jv(_n,-_phi0*cs)*np.exp(-1.0j*_kya)) * np.exp(1.0j*_n*np.pi/2)
  elif _i+1 == _j:
...
for l in range(nky):
  ...
  #          print(Hn[n,i,j])
    ...
    # eigenvalue for k_y
    #print(np.tril(Qmn, k=-1).conj().T)
    Qmn = Qmn + np.tril(Qmn, k=-2).conj().T
    #Qmn = Qmn
    #print(Qmn)
    energy[:,k], states = np.linalg.eig(Qmn)
    ...
    np.savetxt('./data/eigenenergy-phi-%03i.txt' % (k), energy, fmt = '%1.8f')
