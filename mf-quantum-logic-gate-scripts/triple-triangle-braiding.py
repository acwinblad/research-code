...
bdg = np.block( [ [bdgl,zmat,zmat], [zmat,bdgl,zmat], [zmat,zmat,bdgl] ] )

# Link the hopping between the three triangles
bdg[n-1, 3*n-nr] = -t
bdg[2*n-1, 4*n-nr] = t
bdg[3*n-1, 5*n-nr] = -t
bdg[4*n-1, 6*n-nr] = t

# Link the pairing between the three triangles
...
for k, angle in enumerate(tvals):
  ...
  evt[:,k] = eng[3*n-nE:3*n+nE]
  evtt[k] = eng[3*n]
  vec = np.real(np.multiply(vec, np.conj(vec)))
  for i in range(4):
    vgst[i,:,k] = vec[:,3*n-2+i]

htm.plot_triple_hollow_triangle_wavefunction_circles(a, width, nr, coords, tvals, evtt, vgst, filepath)
if(plotSpectral):
